...
void get_param_service_response(
  ...
  const auto hardware_params = createHardwareInfo();
  franka_hardware_interface.on_init(hardware_params);
...
TEST_F(FrankaHardwareInterfaceTest, when_on_init_called_expect_success) {
  auto mock_robot = std::make_shared<MockRobot>();
  const auto info = createHardwareInfo();
  std::string arm_id{"fr3"};
  franka_hardware::FrankaHardwareInterface franka_hardware_interface(mock_robot, arm_id);
  ...
  for (size_t i = 0; i < states.size() - 20; i++) {
    ...
    if (i % 4 == 0) {
      ...
      ASSERT_EQ(states[i].get_name(), joint_name + "/" + "initial_joint_position");
        ...
        ASSERT_EQ(states[i].get_optional<double>().value(), 0.0);
  }
...
TEST_F(
  ...
  EXPECT_NEAR(states[state_interface_size - 19]
              .get_optional<double>().value(),  // initial_pose(16), initial_pose(2) + initial position(7)
...
TEST_F(
  ...
  EXPECT_NEAR(states[state_interface_size - 2].get_optional<double>().value(),
...
TEST_P(FrankaHardwareInterfaceTest,
  ...
  for (size_t i = 0; i < hardware_params.hardware_info.joints.size(); i++) {
